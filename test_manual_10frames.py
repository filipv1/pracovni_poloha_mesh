#!/usr/bin/env python3
"""
Manual test with 10 frames to verify animation works
"""

import subprocess
from pathlib import Path

print("\nMANUAL TEST: 10 FRAMES")
print("="*60)
...
# Write script
script_path = Path("manual_test_10f.py")
...
# Check output
for name in ["manual_test_10f.mp4", "manual_test_10f0001-0010.mp4"]:
    test_file = Path(name)
    if test_file.exists():
        size_mb = test_file.stat().st_size / (1024 * 1024)
        print(f"\n[SUCCESS] {name}: {size_mb:.2f} MB")
        break
else:
