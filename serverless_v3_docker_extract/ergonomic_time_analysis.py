...
class ErgonomicTimeAnalyzer:
    ...
    def run_analysis(self, output_excel="ergonomicka_analyza.xlsx"):
        ...
        try:
            ...
            print(f"\nANALYZA USPESNE DOKONCENA")
            print(f"Excel report: {excel_path}")
            print(f"Celkova delka videa: {len(self.df)/self.fps:.1f} sekund")
...
def main():
    ...
    if result:
        print(f"\nPRIPRAVENO K PROHLEDNUTI:")
        print(f"Otevrete {result} pro detailni ergonomickou analyzu")
        print(f"\nAnalyza obsahuje:")
        print(f"- Cas straveny v jednotlivych uhlovych rozsazich")
        print(f"- Analyzu dlouhodobych poloh (>4 sekundy)")
        print(f"- Procentualni rozdeleni")
        print(f"- Prehledne tabulky pro snadnou interpretaci")
