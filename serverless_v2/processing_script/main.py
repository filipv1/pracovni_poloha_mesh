...
def main(video_path, output_dir, max_frames=None, frame_skip=1, quality='high', gender='neutral', device='auto'):
    """Main execution function, adapted to be called directly."""
    print("PRODUCTION 3D HUMAN MESH PIPELINE (PARALLEL) - Library Mode")
    print("=" * 80)
    ...
    if not models_dir.exists() or not any(models_dir.glob("*.npz")):
        ...
        # In a server context, we might want to raise an exception
        raise FileNotFoundError("SMPL-X models not found.")
    ...
    pipeline = MasterPipeline(smplx_path=models_dir, device=device, gender=gender)
    
    # The execute_parallel_pipeline function returns a dictionary of results
    results = pipeline.execute_parallel_pipeline(
        video_path, output_dir=output_dir, max_frames=max_frames,
        frame_skip=frame_skip, quality=quality
    )
    
    # Return the paths to the generated files
    if results:
        return {
            "pkl_path": str(results.get('mesh_file')),
            "video_path": str(results.get('video_file'))
        }
    return None
